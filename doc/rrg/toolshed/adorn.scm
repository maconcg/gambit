...
(define lambda-bind-mesgs '(lambda-bind case-lambda-bind))
...
(define (need-delim! ac-list nc int/mesg/predicate?)
...
(define (try-nc! ac-list nc)
  (define (adorn-without-context nc)
    (cond ((char=? nc #\\) (adorn-char nc 'infix #f))
          ...
          ((char=? nc #\') (adorn-char nc 'quote #f))
          ((char=? nc #\`) (adorn-char nc 'quasiquote #f))
          ((char=? nc #\,) (adorn-char nc 'unquote 'unquote))
  ...
  (define (adorn-loop! unadorned adorned)
    (define (try-pm! ac-list nc pac)
      ...
      (define (try-bind-pm! ac-list nc pm)
        (and (memq pm bind-mesgs) (if (memc nc delim-chars)
                                      (try-nc! ac-list nc)
                                      (adorn-char nc pm pm))))
      ...
      (define (try-invalid-pm! ac-list nc pm)
        (and (eq? pm 'invalid) (if (memc nc delim-chars)
                                   (try-nc! ac-list nc)
                                   (adorn-char nc 'invalid 'invalid))))
      (define (try-linec-pm! ac-list nc pm)
        (and (eq? pm 'linec) (if (char=? nc #\newline)
                                 (adorn-char #\newline 'whitespace #f)
      ...
      (define (try-rt-syntax-pm! ac-list nc pm)
        (define (handle~rt-syntax! ac-list nc)
          ...
          (let ((tested (append (chars-until ac-list '~~rt-syntax) (list nc))))
        ...
        (or (looking-at ac-list '(lambda-bind lambda-bind-ident-end))
            (let ((up-1 (up-list ac-list)))
              (and (not (operator-position? up-1))
                   (looking-at up-1 '(~~rt-syntax ~rt-syntax))
             ...
             (let* ((up-1 (up-list ac-list))
               ...
               (and (operator-position? up-1-start)
                    (let ((up-2 (up-list up-1-start)))
                      (and (looking-at up-2 '( ~rt-syntax named-let
                                               named-let-ident-end ))
