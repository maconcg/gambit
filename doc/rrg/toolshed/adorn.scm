...
(define (increment-nestc! ac-list nc pac)
  ...
  (adorn-char nc 'nestc 'nestc+))

(define (decrement-nestc! ac-list nc)
  (let ((values (nestc:peer/stack/distance ac-list)))
    (let ((peer (car values)) (stack (cadr values)) (dist (+ (cddr values) 1)))
      (let ((top-peer (caar (get-stack peer)))
            (rest (cdr stack)))
        (set-stack! peer '())
        (increment-hops! rest dist)
        (let ((total-dist (+ (get-hop top-peer) dist))
              (mesg (if (null? rest) 'nestc-end 'nestc-)))
...
(define (handle-nestc nc)
  (cond ((char=? nc #\#) (adorn-char nc 'nestc '~nestc+))
        ((char=? nc #\|) (adorn-char nc 'nestc '~nestc-))
        (else (adorn-char nc 'nestc 'nestc))))

(define (handle~nestc+! ac-list nc pac)
  (cond ((char=? nc #\|) (increment-nestc! ac-list nc pac))
        ((char=? nc #\#) (adorn-char nc 'nestc '~nestc+))
        (else (adorn-char nc 'nestc 'nestc))))

(define (handle~nestc-! ac-list nc)
...
(define (handle-octothorpe! ac-list nc pac)
  (cond ((char=? nc #\\)
        ...
        ((char=? nc #\|)
         ...
         (set-hop! pac 0)
         (set-stack! pac (list ac-list))
         (adorn-char nc 'nestc 'nestc-begin))
        ((char=? nc #\&)
         (set-kind! pac 'box)
         (adorn-char nc 'box #f))
        ((char=? nc #\()
