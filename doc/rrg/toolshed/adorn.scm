...
(define (try-nc! ac-list nc)
  ...
  (cond ((char=? nc #\") (begin-symmetric ac-list nc 'string))
        ...
        ((memc nc (append '(#\- #\+) decimal-chars))
         (cond ((null? ac-list) (adorn-char nc 'number '~dec-num))
               (else (let ((pac (car ac-list)))
                       (if (or (memc (get-char pac) delim-chars)
                               (memq (get-kind pac) (append atmosphere-kinds
                                                            compound-kinds)))
    ...
    (define (try-pm! ac-list nc pac)
      ...
      (define (try-numeric-pm! ac-list nc pac pm)
          ...
          (lambda (ac-list nc pm)
            (and (eq? pm ~mesg#)
                 (cond ((memc-ci nc '(#\e #\i))
                       ...
                       (else (revise-until! ac-list 'default 'octothorpe)
                             (adorn-char nc 'invalid 'invalid))))))
        (define (^try-nondec#num-pm! mesg#num radix-chars)
          (lambda (ac-list nc pm)
            (and (eq? pm mesg#num)
          ...
          (and (eq? pm '~dec-num)
               (cond ((memc nc delim-chars) (try-nc! ac-list nc))
                     ...
                     ((memc-ci nc '(#\e #\s #\f #\d #\l))
                      (set-kind! pac 'number)
                      (adorn-char nc 'number 'dec-precision))
                     ((and (char-ci=? nc #\i) (memc (get-char pac) '(#\- #\+)))
                      (set-kind! pac 'number)
        ...
        (define (try-dec-num-pm! ac-list nc pm)
          (and (eq? pm 'dec-num)
               (cond ((memc nc decimal-chars) (adorn-char nc 'number 'dec-num))
                     ...
                     ((memc-ci nc '(#\e #\s #\f #\d #\l))
                      (adorn-char nc 'number 'dec-precision))
                     (else (revise-until! ac-list 'default '~dec-num)
                           (try-nc! ac-list nc)))))
        (define (try-dec-precision-pm! ac-list nc pm)
          (and (eq? pm 'dec-precision)
               (cond ((memc nc decimal-chars) (adorn-char nc 'number pm))
        ...
        (define (^try~inf/nan-pm! ~mesg mesg strings)
          (lambda (ac-list nc pm)
            (and (eq? pm ~mesg)
                 (let* ((recents (chars-until ac-list '~dec-num))
        ...
        (define try~dec#num-pm!
          (^try~radix-pm! '~dec#num '~dec-num# 'dec#num decimal-chars))
        ...
        (define try~dec-num#-pm! (^try~radix#-pm! '~dec-num# 'dec#num))
        (define try~hex-num#-pm! (^try~radix#-pm! '~hex-num# 'hex#num))
        (define try-bin#num-pm! (^try-nondec#num-pm! 'bin#num binary-chars))
        (define try-oct#num-pm! (^try-nondec#num-pm! 'oct#num octal-chars))
        (define try-hex#num-pm!
          (^try-nondec#num-pm! 'hex#num hexadecimal-chars))
        (or (try~dec-num-pm! ac-list nc pac pm)
    ...
    (define (try-context! ac-list nc pac)
      ...
      (let ((pc (get-char pac)))
        (cond ((not pc) #f)
              ...
              ((memc nc decimal-chars)
               (if (or (memq (get-kind pac) (append atmosphere-kinds
                                                    compound-kinds))
                       (memc pc delim-chars))
                   (adorn-char nc 'number '~dec-num)
                   (adorn-char nc 'default #f)))
