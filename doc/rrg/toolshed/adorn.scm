...
(define (try-ident/string-pm! ac-list nc pac pm)
  ...
  (define (^ident/string-dispatch! base-sym delim-char end!)
    ...
    (define (generic->handler sym)
      ...
      (define (^handle:ident/string-hex-Uu! base-sym delim-char hex-Uu end!)
        (define U-syms '( string-hex-U ident-hex-U datumc-string-hex-U
                          datumc-ident-hex-U ))
        (let ((esc (->esc base-sym)) (backslash (->backslash base-sym)))
          (let ((target (if (memq hex-Uu U-syms) 8 4)))
            (lambda (ac-list nc)
              (cond ((char=? nc delim-char) (end! ac-list nc base-sym))
      ...
      (cond ((memq sym '(base unmatched octal-3))
            ...
            ((memq sym '(hex-U hex-u))
             (^handle:ident/string-hex-Uu! base-sym delim-char sym end!))
            ((eq? sym 'nil-esc)
