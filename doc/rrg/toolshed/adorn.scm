...
(define (^revise-when-complete kind ~mesg strings)
  (lambda (ac-list nc)
    (let* ((tested (append (chars-until ac-list 'octothorpe) (list nc)))
      ...
      (cond (matched
             (revise-until! ac-list kind (- (length matched) 1))
...
(define handle~sharp!
  (^revise-when-complete 'sharp '~sharp sharp-objects))
...
(define (need-delimiter! ac-list nc int/mesg/predicate?)
  (cond ((memc nc delim-chars) (try-nc! ac-list nc))
...
(define (try-bind-pm! ac-list nc pm)
  (define bind-mesgs '( sv-define mv-define defun-proc defun-param named-let
                        sv-let mv-let lambda-bind case-lambda-bind ))
  (and (memq pm bind-mesgs) (cond ((memc nc delim-chars) (try-nc! ac-list nc))
...
(define (try-boolean/hvector-pm! ac-list nc pm)
  (define (handle~f/false/fvector! ac-list nc)
    (cond ((char-numeric? nc)
          ...
          ((memc nc delim-chars) (adorn-without-context nc))
          (else (revise-until! ac-list 'default 2)
                (adorn-char nc 'invalid 'invalid))))
  (define (^handle~hvector! ~mesg prefix-strings)
    ...
    (lambda (ac-list nc)
      (let* ((tested (append (chars-until ac-list 'octothorpe) (list nc)))
        ...
        (cond (matched (let ((kind (hvector-char-list->symbol matched)))
                         (revise-until! ac-list kind (- (length matched) 1))
                         (begin-compound! ac-list nc kind)))
              ((could-match-one-of? prefix-strings tested)
               (adorn-char nc 'default ~mesg))
              ((and (memc nc delim-chars) (not (memc nc compound-begin-chars)))
               (try-nc! ac-list nc))
              (else (adorn-char nc 'invalid 'invalid))))))
  ...
  (cond ((eq? pm '~f/false/fvector) (handle~f/false/fvector! ac-list nc))
        ...
        ((memq pm '(false true))    (need-delimiter! ac-list nc 'octothorpe))
        (else #f)))

(define (try-char-pm! ac-list nc pm)
  (define (finish! ac-list) (revise-until! ac-list 'char 2))
  ...
  (define (^handle~char-Uu! ~mesg mesg)
    (define target-distance (if (eq? mesg 'char-U) 8 4))
    (lambda (ac-list nc)
      (cond ((memc nc delim-chars) (try-nc! ac-list nc))
            ((memc nc hexadecimal-chars)
             (let ((d (distance-until ac-list '~char-1)))
               (when (= d 1) (revise-until! ac-list 'default 3))
               (cond ((> target-distance d) (adorn-char nc 'default ~mesg))
                     (else (revise-until! ac-list 'char-body (+ d 1) finish!)
  ...
  (define (handle~char-2! ac-list nc)
    (cond ((memc nc delim-chars) (try-nc! ac-list nc))
          (else (revise-until! ac-list 'default 3)
                (let ((tested (append (chars-until ac-list 3) (list nc))))
                  (if (could-match-one-of? long-named-chars tested)
                      (adorn-char nc 'default '~char-3)
                      (adorn-char nc 'invalid 'invalid))))))
  (define (handle~char-3! ac-list nc)
    (let ((tested (append (chars-until ac-list 4) (list nc))))
      (cond ((matches-one-of? short-named-chars tested)
             (revise-until! ac-list 'char-body '~char-1 finish!)
  ...
  (cond ((eq? pm '~char-1) (handle~char-1! ac-list nc))
        ...
        ((memq pm '(char-u char-U named-char))
         (need-delimiter! ac-list nc 'octothorpe))
        (else #f)))
...
(define (try-directive/sharp-pm! ac-list nc pm)
  (cond ((eq? pm '~directive)         (handle~directive! ac-list nc))
        ...
        ((memq pm '(directive sharp)) (need-delimiter! ac-list nc 'octothorpe))
        (else #f)))
...
(define (try-invalid-pm! ac-list nc pm)
  (and (eq? pm 'invalid) (cond ((memc nc delim-chars) (try-nc! ac-list nc))
                               (else (adorn-char nc 'invalid 'invalid)))))
...
(define (try-octothorpe-pm! ac-list nc pac pm)
  (and (eq? pm 'octothorpe)
       (cond ((char=? nc #\\) (adorn-char nc 'default '~char-1))
             ((char=? nc #\!) (adorn-char nc 'default '~directive/sharp))
             ((char=? nc #\s) (adorn-char nc 'default '~svector))
             ((char=? nc #\u) (adorn-char nc 'default '~uvector))
             ((char=? nc #\|) (begin-nestc! ac-list nc pac))
             ((memc nc '(#\# #\b #\e #\i #\o #\x)) (adorn-char nc 'default #f))
             ((memc nc decimal-chars) (adorn-char nc 'default '~label/ref))
             ((char=? nc #\f)
              (set-kind! pac 'false)
...
(define (try-runtime-syntax-pm! ac-list nc pm)
  ...
  (and (memq pm '(~runtime-syntax ~~runtime-syntax))
       (handle~runtime-syntax! ac-list nc)))
...
(define (try-unquote-pm! ac-list nc pm)
  (and (eq? pm 'unquote) (cond ((char=? nc #\@) (adorn-char nc 'unquote #f))
                               (else (try-nc! ac-list nc)))))

(define (try-pm! ac-list nc pac)
  (let ((pm (get-mesg pac)))
    (or (try-bind-pm! ac-list nc pm)
        (try-boolean/hvector-pm! ac-list nc pm)
        (try-char-pm! ac-list nc pm)
        ...
        (try-runtime-syntax-pm! ac-list nc pm)
        (try-shebang-pm! ac-list nc pm)
        (try-unquote-pm! ac-list nc pm))))
