...
(define (try-nc! ac-list nc)
  ...
  (define handle-plus/minus (^handle-potential-decimal 'default '~dec/inf/nan))
    ...
    (define (try-pm! ac-list nc pac)
      ...
      (define (try-boolean/hvector-pm! ac-list nc pm)
        ...
        (cond ((eq? pm '~f/false/fvector) (handle~f/false/fvector! ac-list nc))
              ...
              (else #f)))
      (define (try-box-pm! ac-list nc pm)
        (and (eq? pm 'box) (try-nc! ac-list nc)))
        ...
        (define (try?dec?num-pm! ac-list nc pac pm)
          (and (memq pm '(~dec.num dec.num ~dec-num dec-num))
               (cond ((char=? nc #\.)
                     ...
                     ((memc nc delim-chars)
                      (when (and (not (null? ac-list))
                                 (memq pm '(dec-num ~dec-num))
                                 (memc-ci (get-char (cadr ac-list))
                                          '(#\d #\e #\i)))
                        (revise-while! ac-list 'default numeric-kind?))
                      (try-nc! ac-list nc))
          ...
          (and (eq? pm 'dec-precision)
               (cond ((memc nc decimal-chars) (adorn-char nc 'number pm))
                     ...
                     (else (revise-while! ac-list 'default numeric-kind?)
                           (adorn-char nc 'default #f)))))         
        (define try~exact-pm!
          (^try~exactness-pm! '~exact '~exact# 'dec#num))
        (define try~inexact-pm!
          (^try~exactness-pm! '~inexact '~inexact# 'dec#num))
      ...
      (let ((pm (get-mesg pac)))
        (or (try-bind-pm! ac-list nc pac pm)
            ...
            (try-numeric-pm! ac-list nc pac pm)
