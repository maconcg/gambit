      ...
      (let ((from-peer (car p/d)) (distance (cdr p/d)))
        (if from-peer
            (let* ((peer (car from-peer))
              ...
              (let ((ac (adorn-char char kind (->sub-unmatched kind) 0 '())))
                (increment-hops! peer-stack distance)
                (set-stack! peer '())
                (set-stack! ac (cons (cons ac ac-list) peer-stack))
                ac))
            (let ((ac (adorn-char char kind (->unmatched kind) 0 '())))
...
(define (adorn-without-context char)
  (cond ((char=? char #\\) (adorn-char char 'infix #f))
        ...
        ((char=? char #\,) (adorn-char char 'unquote 'unquote))
        ((memc char '(#\space #\newline #\tab))
...
(define (end-datumc-symmetric! ac-list nc kind)
...
(define (handle:unquote ac-list nc)
  (cond ((char=? nc #\@) (adorn-char nc 'unquote #f))
...
(define (try-pm! ac-list nc pac)
  (let ((pm (get-mesg pac)))
    (case pm
      ...
      ((unquote)                  (handle:unquote ac-list nc))
