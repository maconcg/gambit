...
(define token-strings
  (list (cons 'char (map (lambda (s) (string-append "#\\" s)) named-chars))
        (cons 'false '("#f" "#false"))
...
(define (tbd-is? token ac-list)
  (member (preceding-tbd ac-list)
          (cdr (assoc token token-strings symbol=?))
          string=?))
        ...
        (let ((nc (get-char next))
          ...
          (let ((pc (get-char prev))
            ...
            (cond ((and (eq? 'invalid pm) (not (delimiter? nc)))
                  ...
                  ((eq? 'svector pm)
                   (cond ((delimiter? nc)
                          (cond ((and (tbd-is? 'svector rest) (char=? #\( nc))
                                 (revise-kinds! rest 'tbd 'svector)
                                 (set-kind! next 'svector))
                                (else (revise-kinds! rest 'tbd 'invalid))))
                         (else (set-message! next 'svector))))
                  ((eq? 'true pm)
                   (cond ((delimiter? nc)
