  ...
  (case-lambda
    ...
    ((next rest)
     (guard (kind
       ...
       (let* ((pick (lambda (symbol) (raise symbol)))
              (nc (get-char next))
              (prev (car rest))
              (pm (get-message prev)))
         (cond ((eq? 'char pm) 'char)
               ...
               ((eq? 'special-start pm)
                (cond ((ampersand?          nc) (pick 'box))
                      ((backslash?          nc) (pick 'char))
                      ((bar?                nc) (pick 'nested-comment))
                      ((char=? #\f          nc) 'tbd)
                      ((char=? #\t          nc) 'tbd)
                      ((char-in? '(#\s #\u) nc) 'tbd)
                      ((digit10?            nc) 'tbd)
                      ((exactness-char?     nc) (pick 'number))
                      ((exclamation?        nc) 'tbd)
                      ((lparen?             nc) (pick 'vector))
                      ((radix-char?         nc) (pick 'number))
                ...
                (let ((pc (get-char prev)))
                  (if (and (delimiter? nc) (char=? #\t pc))
                      ...
                      (cond
                       ...
                       ((and (char=? #\u pc) (char=? #\e nc)) (pick 'boolean))
