                  ...
                  ((eq? pm 'octothorpe-begin)
                   (cond ((char=? nc #\&)
                         ...
                         ((char=? nc #\!)
                          (set-mesg! nac 'directive-or-sharp))
                         ((exactness-char? nc)
                          (set-mesg! nac 'digit10-or-radix-or-punct))
                         ((char=? nc #\()
                          (set-kind! nac 'vector)
                          (set-mesg! nac adorned-length)
                          (revise-kinds! rest 'vector))
                         ((char=? nc #\x)
                          (set-mesg! nac 'digit16-or-exactness-or-punct))
                         ((char=? nc #\d)
                          (set-mesg! nac 'digit10-or-exactness-or-punct))
                         ((char=? nc #\o)
                          (set-mesg! nac 'digit8-or-exactness-or-punct))
                         ((char=? nc #\b)
                          (set-mesg! nac 'digit2-or-exactness-or-punct))
                         ((char=? nc #\;)
                         ...
                         (else
                          (invalidate! nac))))
                  (else (refresh! nac)))
            (when (and (char=? nc #\)) (not (get-peer nac)))
              (set-mesg! nac adorned-length)
              (peer! nac #\( rest)))))
    nac))
