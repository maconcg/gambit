...
(define (^try-bind-ident-pm! transform)
  (let ((try-sv-define!        (^try-symm-pm! (transform 'sv-define)))
        (try-deflib-r7rs!      (^try-symm-pm! (transform 'deflib-r7rs)))
        ...
        (try-defrec-name!      (^try-symm-pm! (transform 'defrec-name)))
        (try-defrec-cons!      (^try-symm-pm! (transform 'defrec-cons)))
        (try-defrec-param!     (^try-symm-pm! (transform 'defrec-param)))
        (try-defrec-pred!      (^try-symm-pm! (transform 'defrec-pred)))
        (try-defrec-acc!       (^try-symm-pm! (transform 'defrec-acc)))
        (try-defrec-mut!       (^try-symm-pm! (transform 'defrec-mut)))
        (try-guard-bind!       (^try-symm-pm! (transform 'guard-bind)))
        (try-import-lib!       (^try-symm-pm! (transform 'import-lib)))
        (try-sv-let!           (^try-symm-pm! (transform 'sv-let)))
        (try-syntax-let!       (^try-symm-pm! (transform 'syntax-let)))
        (try-sr-ellipsis!      (^try-symm-pm! (transform 'sr-ellipsis)))
        (try-sr-keyword!       (^try-symm-pm! (transform 'sr-keyword)))
        (try-sr-literal!       (^try-symm-pm! (transform 'sr-literal)))
        (try-sr-patvar!        (^try-symm-pm! (transform 'sr-patvar)))
        (try-sr-patvar-op!     (^try-symm-pm! (transform 'sr-patvar-op)))
    ...
    (lambda (ac-list nc pac pm)
      (or (try-sv-define!        ac-list nc pac pm)
          (try-deflib-r7rs!      ac-list nc pac pm)
          (try-defsyntax!        ac-list nc pac pm)
          (try-defun-param!      ac-list nc pac pm)
          (try-defun-proc!       ac-list nc pac pm)
          (try-defrec-name!      ac-list nc pac pm)
          (try-defrec-cons!      ac-list nc pac pm)
          (try-defrec-param!     ac-list nc pac pm)
          (try-defrec-pred!      ac-list nc pac pm)
          (try-defrec-acc!       ac-list nc pac pm)
          (try-defrec-mut!       ac-list nc pac pm)
          (try-guard-bind!       ac-list nc pac pm)
          (try-import-lib!       ac-list nc pac pm)
          (try-sv-let!           ac-list nc pac pm)
          (try-syntax-let!       ac-list nc pac pm)
          (try-sr-ellipsis!      ac-list nc pac pm)
          (try-sr-keyword!       ac-list nc pac pm)
          (try-sr-literal!       ac-list nc pac pm)
          (try-sr-patvar!        ac-list nc pac pm)
          (try-sr-patvar-op!     ac-list nc pac pm)
