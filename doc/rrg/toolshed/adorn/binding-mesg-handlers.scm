(define (^handle~bind! sym ~sym)
  (let ((beginning-kinds (append atmosphere-kinds binding-compounds))
        (ident-sym (->ident sym))
        (inert-ident-sym (->ident (->inert sym)))
...
;==============================================================================
(define try-bind-pm!
  (let ((ident-ends (lambda (sym) (list (->end (->ident sym))
                                        (->end (->ident (->inert sym)))))))
    (let ((defun-proc '(~defun-proc subdefun-unmatched))
          (sv-let-outer (cons '~~~let-sv (ident-ends 'named-let)))
          (sv-let '(~sv-let sublet-sv-inner-unmatched))
          (mv-let-inner '(~~let-mv sublet-mv-outer-unmatched))
          (mv-let (append '(~mv-let sublet-mv-inner-unmatched)
                          (ident-ends 'mv-let)))
          ...
          (defun-param (append '(~defun-param)
                               (ident-ends 'defun-proc)
                               (ident-ends 'defun-param)))
          (lambda-bind (append '(sublambda-bind-list-unmatched ~lambda-bind)
                               (ident-ends 'lambda-bind)))
          (sv-let-inner '(~~let-sv sublet-sv-outer-unmatched
                                   sublet-sv-inner-end))
          (mv-let-outer '(~~~let-mv sublet-mv-outermost-unmatched
                                    sublet-mv-outer-end))
          (mv-define (append '(subdefine-mv-list-unmatched ~mv-define)
                             (ident-ends 'mv-define)))
          (case-lambda-bind (append
                             '(subcase-lambda-inner-unmatched ~case-lambda)
                             (ident-ends 'case-lambda-bind)))
          (defproc-param (append '(subdefproc-inner-end ~defproc-param)
                                 (ident-ends 'defproc-param)
                                 (ident-ends 'defproc-proc)))
          (defproc-spec '(subdefproc-inner-unmatched ~defproc-spec)))
