...
(define (handle:sr-patvar! ac-list nc pac)
  (let ((ellipsis '(#\. #\. #\.))
    ...
    (if (matches? ellipsis tested)
        (begin (revise-until! ac-list 'aux-syntax (- (length ellipsis) 1))
               (adorn-char nc 'aux-syntax 'sr-patvar-ell))
...
(define (handle:sr-patvar-ell! ac-list nc pac)
  (cond ((memc nc delim-chars) (^handle:sr-patvar! ac-list nc pac))
        (else (let ((ellipsis '(#\. #\. #\.)))
                (revise-until! ac-list 'sr-patvar (length ellipsis))
                (^handle:sr-patvar! ac-list nc pac)))))
