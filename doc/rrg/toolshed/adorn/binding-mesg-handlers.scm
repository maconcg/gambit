...
(define (^handle~bind! sym ~sym)
  (let ((beginning-kinds (append atmosphere-kinds binding-compounds))
        ...
        (unquote? (lambda (ac-list)
                    (let ((pac (car ac-list)))
                      (let ((pk (get-kind pac)) (pc (get-char pac)))
                        (or (and (eq? pk 'aux-syntax) (memc pc '(#\, #\@)))
                            (and (looking-at ac-list '('~rt-syntax))
                                 (eq? (rt-syntax-mesg ac-list) 'unquote))))))))
    (let ((inert-ident-sym (->ident inert-sym))
      ...
      (lambda (ac-list nc pac)
        (cond ((memc nc whitespace-chars) (adorn-char nc 'whitespace ~sym))
              ((char=? nc #\|)
               (begin-symmetric ac-list nc (if (unquote? ac-list)
                                               inert-ident-sym
                                               ident-sym)))
              ((memc nc compound-begin-chars)
               (if (unquote? ac-list)
                   ...
                   (try-nc! ac-list nc)))
              ((memc nc '(#\' #\`)) (adorn-char nc 'syntax-abbrev ~sym))
              ((char=? nc #\,) (adorn-char nc 'aux-abbrev ~sym))
              ((and (char=? nc #\@) (char=? (get-char pac) #\,))
               (adorn-char nc 'aux-syntax ~sym))
...
(define ^handle~deflib! (^handle~bind! 'named-deflib '~deflib))
(define (handle~deflib! ac-list nc pac)
  (cond ((memc nc compound-begin-chars)
         (begin-compound! ac-list nc 'deflib-r7rs-list))
        (else (^handle~deflib! ac-list nc pac))))

(define handle~defrec-name! (^handle~bind! 'defrec-name '~defrec-name))
...
(define ^handle~sr-ellips/lit!
  (^handle~bind! 'sr-ellipsis '~sr-ellips/lit))
(define (handle~sr-ellips/lit! ac-list nc pac)
  (cond ((memc nc compound-begin-chars)
         (begin-compound! ac-list nc 'sr-literals))
        (else (^handle~sr-ellips/lit! ac-list nc pac))))

(define ^handle~sr-patvar!
  (^handle~bind! 'sr-patvar '~sr-patvar))
(define (handle~sr-patvar! ac-list nc pac)
  (cond ((memc nc compound-begin-chars)
         (begin-compound! ac-list nc 'sr-subpattern))
        ((and (char=? nc #\_) (memc (get-char pac) delim-chars))
         (adorn-char nc 'aux-syntax 'sr-patvar_))
        (else (^handle~sr-patvar! ac-list nc pac))))

(define ^handle~sr-patvar-op!
  (^handle~bind! 'sr-patvar-op '~sr-patvar-op))
(define (handle~sr-patvar-op! ac-list nc pac)
  (cond ((memc nc compound-begin-chars)
         (begin-compound! ac-list nc 'sr-subpattern))
        ((and (char=? nc #\_) (memc (get-char pac) delim-chars))
         (adorn-char nc 'aux-syntax 'sr-patvar_))
        (else (^handle~sr-patvar-op! ac-list nc pac))))
...
(define ^handle~~import-set! (^handle~bind! 'import-lib '~~import-set))
(define (handle~~import-set! ac-list nc pac)
  (cond ((memc nc compound-begin-chars)
         (begin-compound! ac-list nc 'import-set))
        (else (^handle~~import-set! ac-list nc pac))))
...
(define handle~defrec-cons!  (^handle~bind! 'defrec-cons      '~defrec-cons))
(define handle~defrec-param! (^handle~bind! 'defrec-param     '~defrec-param))
(define handle~defrec-pred!  (^handle~bind! 'defrec-pred      '~defrec-pred))
(define handle~defrec-field! (^handle~bind! 'defrec-field     '~defrec-field))
(define handle~defrec-acc!   (^handle~bind! 'defrec-acc       '~defrec-mut))
(define handle~defrec-mut!   (^handle~bind! 'defrec-mut       '~defrec-mut))
...
(define handle~deflib-r7rs!  (^handle~bind! 'deflib-r7rs      '~deflib-r7rs))
(define handle~guard!        (^handle~bind! 'guard-bind       '~guard))
(define handle~rest-spec!    (^handle~bind! 'rest-spec        '~rest-spec))
(define handle~syntax-let!   (^handle~bind! 'syntax-let       '~syntax-let))
(define handle~sr-keyword!   (^handle~bind! 'sr-keyword       '~sr-keyword))
(define handle~sr-literal!   (^handle~bind! 'sr-literal       '~sr-literal))
...
(define ^handle~import-set! (^handle~bind! '~~import-aux '~import-set))
(define (handle~import-set! ac-list nc pac)
  (let ((nac (^handle~import-set! ac-list nc pac)))
    (cond ((eq? (get-kind nac) '~~import-aux)
           (let ((tested (list nc)))
             (cond ((matches-one-of? runtime-syntax tested)
                    (set-kind! nac 'rt-syntax)
                    (set-mesg! nac '~~rt-syntax))
                   ((could-match-one-of? runtime-syntax tested)
                    (set-kind! nac 'default)
                    (set-mesg! nac '~~rt-syntax))
                   (else (set-kind! nac 'default)))
             nac)))))
...
(define (^handle~outer! next-compound-sym mesg)
  (lambda (ac-list nc pac)
    (cond ((memc nc compound-begin-chars)
          ...
          ((memc nc '(#\' #\`)) (adorn-char nc 'syntax-abbrev mesg))
          ((char=? nc #\,) (adorn-char nc 'aux-abbrev mesg))
          ((and (char=? nc #\@) (char=? (get-char pac) #\,))
           (adorn-char nc 'aux-syntax mesg))
...
(define handle~~sr-literal!  (^handle~outer! 'sr-literals      '~~sr-literal))
(define handle~sr-pattern!   (^handle~outer! 'sr-pattern       '~sr-pattern))
(define handle~sr-template!  (^handle~outer! 'sr-template      '~sr-template))
(define handle~sr-rule!      (^handle~outer! 'sr-rule          '~sr-rule))
...
(define handle~~guard!       (^handle~outer! 'guard-list       '~~guard))
(define handle~~defrec-cons!
  (^handle~outer! 'defrec-cons-list '~~defrec-cons))
(define handle~~defrec-field!
  (^handle~outer! 'defrec-field-list '~~defrec-field))
      ...
      (lambda (ac-list nc pac)
        (let ((pc (get-char pac)) (pk (get-kind pac)))
          (cond ((memc nc whitespace-chars)
                ...
                ((memc nc '(#\' #\`))
                 (adorn-char nc 'syntax-abbrev ~next-mesg))
                ((char=? nc #\,) (adorn-char nc 'aux-abbrev ~next-mesg))
                ((memc nc untested-delims) (try-nc! ac-list nc))
                ((or (eq? pk inert-sym) (and (eq? pk 'aux-syntax)
                                             (memc pc '(#\, #\@))))
                 (adorn-char nc inert-sym inert-sym))
                ((aux-ellipsis? ac-list nc)
                 (revise-until! ac-list 'aux-ellipsis 2)
                 (adorn-char nc 'aux-ellipsis 'aux-ellipsis))
                (else (adorn-char nc sym sym))))))))

(define ^handle:sr-patvar! (^handle:bind! 'sr-patvar '~sr-patvar))
(define handle:sr-patvar!
  (let ((ellipsis '(#\. #\. #\.)))
    (lambda (ac-list nc pac)
      (let ((literals (get-sr-literals ac-list))
            (tested (append (chars-while ac-list 'sr-patvar) (list nc))))
        (cond ((matches? ellipsis tested)
               (revise-until! ac-list 'aux-ellipsis (- (length ellipsis) 1))
               (adorn-char nc 'aux-ellipsis 'sr-patvar-ell))
              ((matches-one-of? literals tested)
               (let ((nac (^handle:sr-patvar! ac-list nc pac)))
                 (revise-while! ac-list 'aux-syntax (lambda (ac)
                                                      (eq? (get-kind ac)
                                                           'sr-patvar)))
                 (set-kind! nac 'aux-syntax)
                 nac))
              ((memc nc delim-chars)
               (^handle:sr-patvar! ac-list nc pac))
              (else (revise-while! ac-list 'sr-patvar (lambda (ac)
                                                        (eq? (get-kind ac)
                                                             'aux-syntax)))
                    (^handle:sr-patvar! ac-list nc pac)))))))

(define ^handle:sr-patvar-op! (^handle:bind! 'sr-patvar-op '~sr-patvar))
(define (handle:sr-patvar-op! ac-list nc pac)
  (let ((literals (get-sr-literals ac-list))
        (tested (append (chars-while ac-list 'sr-patvar-op) (list nc))))
    (cond ((matches-one-of? literals tested)
           (let ((nac (^handle:sr-patvar-op! ac-list nc pac)))
             (revise-while! ac-list 'aux-syntax (lambda (ac)
                                                  (eq? (get-kind ac)
                                                       'sr-patvar-op)))
             (set-kind! nac 'aux-syntax)
          ...
          ((memc nc delim-chars)
           (^handle:sr-patvar! ac-list nc pac))
          (else (revise-while! ac-list 'sr-patvar-op (lambda (ac)
                                                       (eq? (get-kind ac)
                                                            'aux-syntax)))
                (^handle:sr-patvar-op! ac-list nc pac)))))

(define (handle:aux-ellipsis! ac-list nc)
  (cond ((memc nc delim-chars) (try-nc! ac-list nc))
        (else (let ((alternate-ac (car (adorn! nc (cdddr ac-list)))))
                (revise-until! ac-list (get-kind alternate-ac) 3)
                alternate-ac))))

(define (handle:sr-patvar_! ac-list nc pac)
  (unless (memc nc delim-chars) (set-kind! pac 'sr-patvar))
  (^handle:sr-patvar! ac-list nc pac))

(define (handle:sr-patvar-ell! ac-list nc pac)
  (cond ((memc nc delim-chars) (^handle:sr-patvar! ac-list nc pac))
        (else (revise-until! ac-list 'sr-patvar 3)
              (^handle:sr-patvar! ac-list nc pac))))
...
(define handle:defun-param!    (^handle:bind! 'defun-param    '~defun-param))
(define handle:defrec-name!    (^handle:bind! 'defrec-name    '~~defrec-cons))
(define handle:defrec-cons!    (^handle:bind! 'defrec-cons    '~defrec-param))
(define handle:defrec-param!   (^handle:bind! 'defrec-param   '~defrec-param))
(define handle:defrec-field!   (^handle:bind! 'defrec-field   '~defrec-acc))
(define handle:defrec-acc!     (^handle:bind! 'defrec-acc     '~defrec-mut))
(define handle:defrec-mut!     (^handle:bind! 'defrec-mut     #f))
(define handle:defsyntax!      (^handle:bind! 'defsyntax      #f))
(define handle:import-lib!     (^handle:bind! 'import-lib     '~~import-set))
(define handle:named-let!      (^handle:bind! 'named-let      '~~~let-sv))
(define handle:named-deflib!   (^handle:bind! 'named-deflib   #f))
(define handle:deflib-r7rs!    (^handle:bind! 'deflib-r7rs    '~deflib-r7rs))
(define handle:syntax-let!     (^handle:bind! 'syntax-let     #f))
(define handle:sr-ellipsis!    (^handle:bind! 'sr-ellipsis    '~~sr-literal))
(define handle:sr-keyword!     (^handle:bind! 'sr-keyword     '~sr-patvar))
(define handle:sr-literal!     (^handle:bind! 'sr-literal     '~sr-literal))
...
(define try-bind-pm!
  (let ((ends (lambda (sym) (list (->end (->ident sym))
    ...
    (let ((~defun-param-mesgs
           (append '(~defun-param) (ends 'defun-proc) (ends 'defun-param)))
          (~defun-proc-mesgs '(~defun-proc subdefun-unmatched))
          (~defrec-cons-mesgs '(~defrec-cons subdefrec-cons-list-unmatched))
          (~defrec-param-mesgs (cons '~defrec-param (ends 'defrec-cons)))
          (~defrec-pred-mesgs '(~defrec-pred subdefrec-cons-list-end))
          (~~defrec-field-mesgs '(~~defrec-field subdefrec-field-list-end))
          (~defrec-field-mesgs '(~defrec-field subdefrec-field-list-unmatched))
          (~defrec-acc-mesgs (cons '~defrec-acc (ends 'defrec-field)))
          (~defrec-mut-mesgs (cons '~defrec-mut (ends 'defrec-acc)))
          (~deflib-r7rs-mesgs '(~deflib-r7rs subdeflib-r7rs-list-unmatched))
          (~syntax-let-outer-mesgs '(~~~let-syntax))
          (~syntax-let-mesgs '(~syntax-let sublet-syntax-inner-unmatched))
          (~sr-literal-outer-mesgs '(~~sr-literal))
          (~sr-literal-mesgs '(~sr-literal subsr-literals-unmatched))
          (~sr-patvar-op-mesgs '(~sr-patvar-op subsr-subpattern-unmatched))
          (~sr-patvar-mesgs '(~sr-patvar subsr-subpattern-end))
          (~sr-rule-mesgs '(~sr-rule subsr-rule-end subsr-literals-end))
          (~sr-keyword-mesgs '(~sr-keyword subsr-pattern-unmatched))
          (~sr-pattern-mesgs '(~sr-pattern subsr-rule-unmatched))
          (~sr-template-mesgs '(~sr-template subsr-pattern-end))
          ...
          (~import-set-mesgs '(~import-set subimport-set-unmatched))
          (~rest-spec-mesgs '(subrest-spec-list-unmatched ~rest-spec))
          (defrec-name-mesgs (bind-mesgs 'defrec-name))
          (defrec-cons-mesgs (bind-mesgs 'defrec-cons))
          (defrec-param-mesgs (bind-mesgs 'defrec-param))
          (defrec-pred-mesgs (bind-mesgs 'defrec-pred))
          (defrec-field-mesgs (bind-mesgs 'defrec-field))
          (defrec-acc-mesgs (bind-mesgs 'defrec-acc))
          (defrec-mut-mesgs (bind-mesgs 'defrec-mut))
          (deflib-r7rs-mesgs (bind-mesgs 'deflib-r7rs))
          (named-deflib-mesgs (bind-mesgs 'named-deflib))
          ...
          (sr-keyword-mesgs (bind-mesgs 'sr-keyword))
          (sr-literal-mesgs (bind-mesgs 'sr-literal))
          (sr-patvar-mesgs (bind-mesgs 'sr-patvar))
          (sr-patvar-ell-mesgs (bind-mesgs 'sr-patvar-ell))
          (sr-patvar-op-mesgs (bind-mesgs 'sr-patvar-op))
          (sr-ellipsis-mesgs (bind-mesgs 'sr-ellipsis))
          ...
          (guard-mesgs (bind-mesgs 'guard-bind))
      ...
      (lambda (ac-list nc pac pm)
        (cond
         ...
         ((memq pm defrec-name-mesgs)   (handle:defrec-name!   ac-list nc pac))
         ((memq pm defrec-cons-mesgs)   (handle:defrec-cons!   ac-list nc pac))
         ((memq pm defrec-param-mesgs)  (handle:defrec-param!  ac-list nc pac))
         ((memq pm defrec-pred-mesgs)   (handle:defrec-pred!   ac-list nc pac))
         ((memq pm defrec-field-mesgs)  (handle:defrec-field!  ac-list nc pac))
         ((memq pm defrec-acc-mesgs)    (handle:defrec-acc!    ac-list nc pac))
         ((memq pm defrec-mut-mesgs)    (handle:defrec-mut!    ac-list nc pac))
         ...
         ((memq pm sr-ellipsis-mesgs)   (handle:sr-ellipsis!   ac-list nc pac))
         ((memq pm sr-keyword-mesgs)    (handle:sr-keyword!    ac-list nc pac))
         ((memq pm sr-literal-mesgs)    (handle:sr-literal!    ac-list nc pac))
         ((memq pm sr-patvar-mesgs)     (handle:sr-patvar!     ac-list nc pac))
         ((memq pm sr-patvar-ell-mesgs) (handle:sr-patvar-ell! ac-list nc pac))
         ((memq pm sr-patvar-op-mesgs)  (handle:sr-patvar-op!  ac-list nc pac))
         ((eq? pm 'sr-patvar_)          (handle:sr-patvar_!    ac-list nc pac))
         ((eq? pm 'aux-ellipsis)        (handle:aux-ellipsis!  ac-list nc))
         ((memq pm sv-let-mesgs)        (handle:sv-let!        ac-list nc pac))
         ((memq pm named-let-mesgs)     (handle:named-let!     ac-list nc pac))
         ((memq pm named-deflib-mesgs)  (handle:named-deflib!  ac-list nc pac))
         ((memq pm deflib-r7rs-mesgs)   (handle:deflib-r7rs!   ac-list nc pac))
         ...
         ((memq pm guard-mesgs)         (handle:guard!         ac-list nc pac))
         ...
         ((eq? pm '~~import-set)        (handle~~import-set!   ac-list nc pac))
         ((eq? pm '~sv-define)          (handle~sv-define!     ac-list nc pac))
         ((eq? pm '~defrec-name)        (handle~defrec-name!   ac-list nc pac))
         ((eq? pm '~deflib)             (handle~deflib!        ac-list nc pac))
         ((eq? pm '~defsyntax)          (handle~defsyntax!     ac-list nc pac))
         ((eq? pm '~named/sv-let)       (handle~named/sv-let!  ac-list nc pac))
         ((eq? pm '~sr-ellips/lit)      (handle~sr-ellips/lit! ac-list nc pac))
         ((eq? pm '~~~~let-mv)          (handle~~~~let-mv!     ac-list nc pac))
         ((eq? pm '~~defproc)           (handle~~defproc!      ac-list nc pac))
         ((eq? pm '~~guard)             (handle~~guard!        ac-list nc pac))
         ((eq? pm '~~mv-define)         (handle~~mv-define!    ac-list nc pac))
         ((eq? pm '~~defrec-cons)       (handle~~defrec-cons!  ac-list nc pac))
         ((memq pm ~defun-param-mesgs)  (handle~defun-param!   ac-list nc pac))
         ((memq pm ~deflib-r7rs-mesgs)  (handle~deflib-r7rs!   ac-list nc pac))
         ...
         ((memq pm ~sv-let-mesgs)       (handle~sv-let!        ac-list nc pac))
         ((memq pm ~sr-literal-mesgs)   (handle~sr-literal!    ac-list nc pac))
         ((memq pm ~sr-pattern-mesgs)   (handle~sr-pattern!    ac-list nc pac))
         ((memq pm ~sr-patvar-mesgs)    (handle~sr-patvar!     ac-list nc pac))
         ((memq pm ~sr-patvar-op-mesgs) (handle~sr-patvar-op!  ac-list nc pac))
         ((memq pm ~sr-rule-mesgs)      (handle~sr-rule!       ac-list nc pac))
         ((memq pm ~sr-template-mesgs)  (handle~sr-template!   ac-list nc pac))
         ...
         ((memq pm ~guard-mesgs)        (handle~guard!         ac-list nc pac))
         ((memq pm ~rest-spec-mesgs)    (handle~rest-spec!     ac-list nc pac))
         ((memq pm ~import-set-mesgs)   (handle~import-set!    ac-list nc pac))
         ...
         ((memq pm ~sr-keyword-mesgs)
          (handle~sr-keyword! ac-list nc pac))
         ((memq pm ~sr-literal-outer-mesgs)
          (handle~~sr-literal! ac-list nc pac))
         (else #f))))))
