...
(define (rt-syntax? ac)
  (memq (get-kind ac) '(rt-syntax aux-syntax)))
                 ...
                 (cond ((memc nc whitespace-chars)
                       ...
                       ((memc nc etc-delims)
                        (when (matches? '(#\# #\#) recents)
                          (revise-until! ac-list 'repl-ref '2))
                        (try-nc! ac-list nc))
                       ((matches? '(#\e #\l #\s #\e) tested)
                        (let* ((up1 (up-list ac-list))
                               (up1-syntax-mesg (rt-syntax-mesg up1)))
                          (cond ((or (eq? up1-syntax-mesg 'rt-cond-aux)
                                     (eq? (rt-syntax-mesg (up-list up1))
                                          '~~guard))
                                 (revise-until! ac-list 'aux-syntax 3)
                                 (adorn-char nc 'aux-syntax '~rt-syntax))
                                (else (adorn-char nc 'default '~rt-syntax)))))
                       ((matches? '(#\u #\n #\q #\u #\o #\t #\e) tested)
                        (revise-until! ac-list 'aux-abbrev 6)
                        (adorn-char nc 'aux-abbrev '~rt-syntax))
                       ((matches? '(#\u #\n #\q #\u #\o #\t #\e #\-
                                    #\s #\p #\l #\i #\c #\i #\n #\g)
                                  tested)
                        (revise-until! ac-list 'aux-abbrev 16)
                        (adorn-char nc 'aux-abbrev '~rt-syntax))
                       ((matches? '(#\i #\m #\p #\o #\r #\t) tested)
                        (let* ((up1 (up-list ac-list))
                               (up1-syntax-mesg (rt-syntax-mesg up1)))
                          (cond
                           ((eq? up1-syntax-mesg '~deflib)
                            (revise-until! ac-list 'aux-syntax '~~rt-syntax)
                            (set-mesg! (car ac-list) '~~import-set)
                            (adorn-char nc 'aux-syntax '~rt-syntax))
                           (else
                       ...
                       ((matches? '(#\r #\e #\n #\a #\m #\e) tested)
                        (let* ((up1 (up-list ac-list))
                               (up1-syntax-mesg (rt-syntax-mesg up1)))
                          (cond
                           ((eq? up1-syntax-mesg '~~import-set)
                            (revise-until! ac-list 'aux-syntax '~~rt-syntax)
                            (set-mesg! (car ac-list) '~~import-set)
                            (adorn-char nc 'aux-syntax '~rt-syntax))
                           ((eq? up1-syntax-mesg '~export-set)
                            (revise-until! ac-list 'aux-syntax '~~rt-syntax)
                            (adorn-char nc 'aux-syntax '~rt-syntax))
                           (else (revise-while! ac-list 'default rt-syntax?)
                                 (adorn-char nc 'default #f)))))
                       ((matches-one-of? import-aux-syntax tested)
                        (let ((up1 (up-list ac-list)))
                          (cond
                           ((eq? (rt-syntax-mesg up1) '~~import-set)
                            (revise-until! ac-list 'aux-syntax '~~rt-syntax)
                            (set-mesg! (car ac-list) '~~import-set)
                            (adorn-char nc 'aux-syntax '~rt-syntax))
                           (else (revise-while! ac-list 'default rt-syntax?)
                                 (adorn-char nc 'default #f)))))
                       ((matches-one-of? define-library-aux-syntax tested)
                        (let* ((up1 (up-list ac-list))
                               (up1-syntax-mesg (rt-syntax-mesg up1)))
                          (cond
                           ((eq? up1-syntax-mesg '~deflib)
                            (revise-until! ac-list 'aux-syntax '~~rt-syntax)
                            (when (matches? '(#\e #\x #\p #\o #\r #\t) tested)
                              (set-mesg! (car ac-list) '~export-set))
                            (adorn-char nc 'aux-syntax '~rt-syntax))
                           ((matches-one-of? define-library-only-aux-syntax
                                             tested)
                            (revise-while! ac-list 'default rt-syntax?)
                            (adorn-char nc 'default #f))
                           (else
                            (revise-until! ac-list 'rt-syntax '~~rt-syntax)
                            (set-mesg! (car ac-list) (syntax->mesg tested))
                            (adorn-char nc 'rt-syntax '~rt-syntax)))))
