...
(define (in-syntax-rules? ac-list)
  (and (not (null? ac-list))
       (let ((mesg (rt-syntax-mesg ac-list)))
         (or (eq? mesg '~sr-ellips/lit)
             (in-syntax-rules? (up-list ac-list))))))

(define (aux-ellipsis? ac-list nc)
  (and (char=? nc #\.)
       (not (null? ac-list))
       (let ((dr (cdr ac-list)))
         (and (not (null? dr))
              (let ((pac (car dr)))
                (and (char=? (get-char pac) #\.)
                     (let ((ddr (cdr ac-list)))
                       (and (not (null? ddr))
                            (let ((ppac (car ddr)))
                              (and (char=? (get-char ppac) #\.)
                                   (let ((dddr (cdr dr)))
                                     (and (not (null? dddr))
                                          (let ((pppac (car dddr)))
                                            (and (memc (get-char pppac)
                                                       delim-chars)
                                                 (in-syntax-rules?
                                                  ac-list)))))))))))))))
      ...
      (and (not (null? rest))
           (let* ((ac (car rest)) (mesg (get-mesg ac)))
             (cond ((eq? mesg 'linec-end)
                    (let ((ac-stack (get-stack ac)))
                      (if (null? ac-stack)
                          (looking/peeking-loop (cdr rest))
